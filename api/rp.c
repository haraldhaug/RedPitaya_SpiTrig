...
/* HH apr 22, 2017
  added data structure elements and interface functions to support SPI simulation and SPI trigger
*/
...
// HH 2017-03-21 add SPI section

int rp_spi_SetSimFlag(uint32_t newval)
{
	iowrite32(newval, &hk->sim_flag);
    return RP_OK;
}

int rp_spi_SetSimBits(uint32_t newval)
{
	iowrite32(newval, &hk->sim_bits);
    return RP_OK;
}

int rp_spi_SetSimPeriod(uint32_t newval)
{
	iowrite32(newval, &hk->sim_period);
    return RP_OK;
}

int rp_spi_SetMosi(uint32_t mask, uint32_t pattern)
{
	iowrite32(mask, &hk->tr_mosi_mask);
    iowrite32(pattern, &hk->tr_mosi);
    return RP_OK;
}

int rp_spi_SetMisoFlag(uint32_t newval)
{
	iowrite32(newval, &hk->tr_miso_flag);
    return RP_OK;
}

int rp_spi_SetMiso(uint32_t mask, uint32_t pattern)
{
	iowrite32(mask, &hk->tr_miso_mask);
    iowrite32(pattern, &hk->tr_miso);
    return RP_OK;
}

// HH 2017-03-21 end of SPI section
